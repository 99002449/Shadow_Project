...
Window {
    ...
    function do_it(referenceToMyGraph,number)
    {
        ...
        console.log("In do_it ( )  id:  "+ referenceToMyGraph)
        ...
        if(number===0)
        {
            ...
            globalSeries =series
            console.log("In number ===0   "+ "SERIES: " +series  )
        ...
        if(number===1)
        {
            series1 =  referenceToMyGraph.referencetoMyChart.createSeries(MyGraphChart2.SeriesTypeSpLine, "Wavefom 2", referenceToMyGraph.referencetoXAxis, referenceToMyGraph.referencetoYAxis);
            globalSeries =series1
            console.log("In number ===1")
            series1.pointsVisible = true;
            series1.color = "white"
            series1.hovered.connect(function(point, state){ console.log(point); }); // connect onHovered signal to a function
        }
...
if(temp >= total && total!==0)
    ...
    function plotGraph()
    {
        console.log( "in plotgraph SERIES: " +globalSeries )
        var total = mywaveform.numberOfSamples();
        if(mywaveform.numberOfSamples()!==0)
        {
            for(var j = 0;j < total;j++)
            {
                ...
                // console.log("x y point appended")
            }
        }
    }
