...
Window {
    ...
    function do_it()
    {
        ...
        if(total!==0)
        {
            ...
            yAxis.min=ymin_temp*2;
            yAxis.max=ymax_temp*2;
            ...
            for(var j = 0;j < temp;j++)
            {
                ...
                var y = mywaveform.getY(j)*amplitudeDial.value;
                series.append(x, y);
            }
            xAxis.max+=samples;
            xAxis.min+=samples;
    ...
    function setupp()
    {  var amplitudeDial1=graphSettings.referenceToAmplitudeDial
         yAxis= graph.referenceToYAxis
         xAxis = graph.referenceToXAxis
        ...
        yAxis.min=(mywaveform.ymin()-1)*2;
        yAxis.max=(mywaveform.ymax()+1)*2;
